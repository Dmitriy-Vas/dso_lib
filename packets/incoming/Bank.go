...
type BankPacket struct {
	...
	Variable1 int32
	Slot      byte
	Items     []dso_lib.InvItemRec
}

func (packet *BankPacket) Read(b buffer.PacketBuffer) {
	packet.Variable1 = b.ReadInt(b.Bytes(), b.Index())
	packet.Slot = b.ReadByte(b.Bytes(), b.Index())
	packet.Items = make([]dso_lib.InvItemRec, 48+packet.Slot)
	if packet.Slot != 0 {
		packet.Items[0] = dso_lib.ReadPlayerBankData(b)
	} else {
		for i := range packet.Items {
			packet.Items[i] = dso_lib.ReadPlayerBankData(b)
		}
	}
}
