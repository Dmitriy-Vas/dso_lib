...
func (packet *AwardsPacket) Read(b buffer.PacketBuffer) {
	...
	if packet.AwardID != 0 {
		...
		packet.Awards[0].Level = make([]dso_lib.AwardsLevelRec, packet.Awards[0].LevelCount)
		...
		packet.Awards = make([]dso_lib.AwardsRec, 200) // TODO int to const
		for i := range packet.Awards {
			...
			packet.Awards[i].Level = make([]dso_lib.AwardsLevelRec, packet.Awards[i].LevelCount)
