...
func (packet *ClassesDataPacket) Read(b buffer.PacketBuffer) {
	...
	for i := range packet.Classes {
		...
		packet.Classes[i].HairTint = make([]dso_lib.ClassHairTintRec, hairTintAmount)
		...
		packet.Classes[i].Sprite = make([]dso_lib.ClassSpriteRec, classSpriteAmount)
		...
		packet.Classes[i].Hair = make([]dso_lib.ClassSpriteRec, classHairAmount)
