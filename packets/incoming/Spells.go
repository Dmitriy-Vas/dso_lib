...
type SpellsPacket struct {
	...
	Variable1    bool
	SpellNum     int32
	PlayerSpells []dso_lib.PlayerSpellRec
}
...
func (packet *SpellsPacket) Read(b buffer.PacketBuffer) {
	...
	packet.PlayerSpells = make([]dso_lib.PlayerSpellRec, 42) // TODO move to constants
	for i := range packet.PlayerSpells {
		packet.PlayerSpells[i] = dso_lib.PlayerSpellRec{
